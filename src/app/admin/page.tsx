...
export default async function AdminDashboardPage() {
  ...
  const [{ data: fieldsData }, { data: bookingsData }, { data: paymentsData }] = await Promise.all([
    ...
    supabase.from('bookings').select('id, field_id, booking_date, start_time, end_time, status, price, fields(name), profiles(name)').order('booking_date', { ascending: false }).limit(200),
